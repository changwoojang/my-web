;(function($){
	...
	$.fn.niceTree = function(method, options) {
			...
			cookie: function(action, id, ul, cookies) {
				if (cookies) {
					if (action === 'set') {
						...
						$.cookie('nicetree-' + id, escape(ul), {path: '/'});
					} else {
		...
		var methods = {
			...
			select: function(options) {
				...
				$tree.find('a').each(function() {
					if(options == $(this).attr("href")) {	
						console.log("[nice-tree] select : " + $(this).attr("href"));
						$(this).show().parent('li').addClass('active').parent('ul').show().parent('li').addClass('active').parent('ul').show();
					}
				});
			}
		};
